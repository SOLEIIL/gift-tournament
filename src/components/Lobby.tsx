...
export const Lobby: React.FC<LobbyProps> = ({
  ...
  // Utiliser le service Telegram pour récupérer l'utilisateur et ses gifts
  const { user, userGifts, isLoading: telegramLoading, getUserDisplayName, getUserInitials } = useTelegramService();
  
  // Utiliser l'utilisateur Telegram comme joueur actuel
  const currentPlayer = user ? {
    id: `telegram_${user.id}`,
    name: getUserDisplayName(),
    shortName: getUserInitials(),
    nfts: userGifts,
    totalValue: userGifts.reduce((sum, nft) => sum + nft.value, 0),
    winChance: 0,
  } : players[0]; // Fallback vers le premier joueur mock
  ...
  const addTestPlayer1 = () => {
    ...
    const availableNFTs = userGifts.slice(0, 3);
  ...
  const addTestPlayer2 = () => {
    ...
    const availableNFTs = userGifts.slice(3, 5);
    availableNFTs.forEach(nft => {
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <QuickDeposit 
          availableNFTs={userGifts.filter(nft => !currentPlayerNFTs.some(deposited => deposited.id === nft.id))}
