...
🎯 Votre inventaire est synchronisé en temps réel !`;
  ...
  async showInventory(chatId, userId, username) {
    try {
      // 🔄 Récupérer l'inventaire depuis Supabase (avec fallback vers l'inventaire virtuel)
      let inventory;
      try {
        console.log(`🔄 Récupération inventaire Supabase pour: ${userId}`);
        inventory = await SupabaseInventoryManager.getUserInventory(userId);
        console.log(`✅ Inventaire Supabase récupéré: ${inventory.length} gifts`);
      } catch (supabaseError) {
        console.log('🔄 Fallback vers l\'inventaire virtuel:', supabaseError.message);
        inventory = this.virtualInventory.getUserInventory(userId);
      }
  ...
  async showStats(chatId, userId, username) {
    try {
      ...
      let inventory;
      try {
        console.log(`🔄 Récupération inventaire Supabase pour les stats: ${userId}`);
        inventory = await SupabaseInventoryManager.getUserInventory(userId);
        console.log(`✅ Inventaire Supabase récupéré pour les stats: ${inventory.length} gifts`);
      } catch (supabaseError) {
        console.log('🔄 Fallback vers l\'inventaire virtuel pour les stats:', supabaseError.message);
        inventory = this.virtualInventory.getUserInventory(userId);
      }
